...
export function FloatingScene() {
  ...
  if (prefersReduced) {
    return (
      <div className="w-64 h-64 rounded-full bg-gradient-to-tr from-accent-purple via-accent-blue to-accent-cyan filter blur-[32px] opacity-40 animate-pulse-slow mx-auto" />
    );
  }
  ...
  return (
    <div className="w-full h-[320px] md:h-[450px] relative select-none">
      ...
      <div className="absolute inset-0 bg-accent-purple/10 rounded-full filter blur-[100px] w-[60%] h-[60%] left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none" />
      
      <Canvas camera={{ position: [0, 0, 4], fov: 45 }}>
        <ambientLight intensity={0.5} />
        <directionalLight position={[2, 3, 2]} intensity={1.5} color="#38BDF8" />
